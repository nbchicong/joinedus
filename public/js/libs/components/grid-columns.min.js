...
UI.grid.Editor,{render:function(a,b){var c=this.getColumn();if(BaseUI.isObject(c)){var e=c.separate||"&nbsp;",d=c.buttons||[],f=b||{},g=$("<div></div>");if(BaseUI.isArray(d))for(var h=0;h<d.length;h++){var i=d[h];i.id="btn-"+BaseUI.generateId();var m=i.visibled||function(){return true},j=m(f),l=i.labelCls!=undefined?i.labelCls:"label label-info";j||(l+=" hide");var k=$(String.format('<span title="{1}" class="{2}"><i class="{0} icon-white"/></span>',i.icon,i.text,l));g.append(k);k.on("click",function(){d[$(this).index()].fn(f)});
...
a)));return this.getEl()},edit:function(a){var b=this.getColumn(),c=b.disabled?'disabled="disabled"':"",e=b.editData||[],d=[],f=b.valueField||"value",g=b.displayField||"text",h=b.emptyString||"----------",i=!BaseUI.isEmpty(b.forceSelection)?b.forceSelection:true,m=BaseUI.isObject(a)?a[g]:"";a=BaseUI.isObject(a)?a[f]:a;var j=null,l=this.getGrid();j=false;i||d.push(String.format('<option value="">{0}</option>',h));for(h=0;h<e.length;h++){var k=e[h]||{};if(k[f]==a){d.push(String.format('<option selected value="{0}">{1}</option>',
k[f],k[g]));j=true}else d.push(String.format('<option value="{0}">{1}</option>',k[f],k[g]))}if(!j&&!BaseUI.isEmpty(a))d.splice(0,0,String.format('<option selected value="{0}">{1}</option>',a,m||a));j=b.type=="select2"?$(String.format('<select {0} data-type="select2" style="width:100%">',c)+d.join()+"<select/>"):$(String.format('<select data-type="select" {0}>',c)+d.join()+"<select/>");j.on("change",function(){l.changeCell(this)}.createDelegate(this));b.type=="select"&&j.on("click",function(){l.clickCell(this)}.createDelegate(this));
this.setEl(j);a=this.applyUI();b.type=="select2"&&a&&a&&a.on("select2-opening",function(){l.clickCell(this)}.createDelegate(this));return this.getEl()},applyUI:function(){var a=this.getColumn()||{};if(a.type=="select2"){if(!this.getEl())throw Error("$cell must not be null");try{if($.fn.select2)return this.getEl().select2(a.getConfig()||{});else console.error("select2 plugin could not be found")}catch(b){throw Error(b.message);}}},getValue:function(){var a=this.getColumn(),b=this.getEl();switch(a.type){case "select":var c=
