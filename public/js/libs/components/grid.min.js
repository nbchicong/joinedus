...
c){this.options.dataOptions.filter=c;this.renderData()},goPage:function(b){var c=b||0;this.options.dataOptions.pageIndex=c;if(this.remotePaging){var g=this.getParams();if(!BaseUI.isEmpty(g))g[this.getPageProperty()]=this.options.dataOptions.pageIndex;this.setParams(g);this.load()}else this.renderData()},previous:function(){if(!(this.paging.page<=1)){this.options.dataOptions.pageIndex--;this.goPage(this.options.dataOptions.pageIndex)}},next:function(){if(!(this.paging.page>=this.paging.pages)){this.options.dataOptions.pageIndex++;
